...
mod idea2 {
    ...
    pub trait InsertLink1 {
        type PreOp: OperationOutput;
        fn pre_op(&self, split_input: ()) -> Self::PreOp;
    }

    pub trait InsertLink2: InsertLink1 {
        type OnInsertInput; // whether this is from split or preop I don't care
        type OnInsertExpr;
        fn on_insert(&self, input: Self::OnInsertInput) -> Self::OnInsertExpr;
    }

    pub trait InsertLink3 {
        type FromRow: FromRowData;
        fn from_row_handler(&self) -> Self::FromRow;

        type FromRowTakeInput;
        type FromRowPostOpInput;
        fn from_row(
            &self,
            from_row_handler_input: <Self::FromRow as FromRowData>::RData,
        ) -> (Self::FromRowTakeInput, Self::FromRowPostOpInput);
    }

    pub trait InsertLink4: InsertLink3 {
        type PostOp: OperationOutput;
        fn post_op(
            &self,
            split_input: (),
            from_row_post_op_input: Self::FromRowPostOpInput,
        ) -> Self::PostOp;
    }

    pub trait InsertLink5: InsertLink3 + InsertLink4 {
        type Output;
        fn take(
            self,
            post_op: <Self::PostOp as OperationOutput>::Output,
            from_row_take_input: Self::FromRowTakeInput,
        ) -> Self::Output;
    }
}
...
mod idea1 {
    ...
    pub trait InsertLink2: InsertLink1 {
        ...
        fn on_insert(
            &self,
            split_input: (),
            insert_1_input: <Self::PreOp as OperationOutput>::Output,
        ) -> Self::OnInsertExpr;
    }
...
impl og_idea::InsertLinkSplit for () {
    type LinkSpec = ();
    type PreOpInput = ();
...
impl<H, LSpec, L> OperationOutput for InsertOne<H, H::InputData, L>
where
    L: InsertLinkSplit<LinkSpec = LSpec>,
    LSpec: InsertLink,
    H: Collection,
{
    type Output = LinkedOutput<<H::Id as CollectionId>::IdData, H::OutputData, LSpec::Output>;
}

impl<S, Base, L, Link, PreOpInput, InsertItemsInput, PostOpInput> Operation<S>
    for InsertOne<Base, Base::InputData, L>
where
    S: DatabaseExt,
    S: ExecutorTrait,
    L: Send,
