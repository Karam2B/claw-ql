...
pub mod sqlx_type_ident {
    ...
    impl<S> Clone for Box<dyn SqlxTypeHandler<S> + Send> {
        fn clone(&self) -> Self {
            self.clone_self()
        }
    }
...
#[claw_ql_macros::skip]
/// this is premature abstraction, I'm thinking of deleting the whole thing
pub mod json_dynamic_link {
    use crate::{
        json_client::fetch_one::JsonLinkFetchOne,
    ...
    impl<S> CollectionsStore for Arc<dyn JsonCollection<S> + Send + Sync + 'static> {
    ...
    where
        S: Database,
        T: DynamicLink<Arc<dyn JsonCollection<S> + Send + Sync + 'static>>,
...
pub mod dynamic_collection {
    ...
    mod str_aliased_impls {
        ...
        impl<S> StrAliased for DynamicCollection<S> {
            type StrAliased = DynamicAliasedCols;
...
pub mod fetch_many {
    ...
    use crate::{
        json_client::{
            database_for_json_client::DatabaseForJsonClient,
            fetch_many::json_link_fetch_many_extention::JsonLinkFetchMany,
    ...
    pub type FetchManyOutput =
        ManyOutput<LinkedOutput<i64, serde_json::Value, serde_json::Value>, (i64, ())>;
                    ...
                    for each in input.links {
                        match each {
                            SupportedLinkFetchMany::OptionalToMany(to) => {
                                ...
                                let to = if let Some(e) = self.collections.get(to.as_str()) {
                                    e.read().await.clone()
                                } else {
                                    panic!();
                                };
    ...
    pub mod json_link_fetch_many_extention {
        ...
        use crate::{
            ...
            database_extention::DatabaseExt,
            debug_any::DebugAny,
            expressions::single_col_expressions::AliasedCol,
            extentions::common_expressions::{
                StrAliased,
                dyn_from_row::DynFromRow,
            ...
            query_builder::{
                Expression, ManyBoxedExpressions, ManyExpressions, SanitizeMany,
                functional_expr::BoxedExpression,
            },
        };

        pub struct JsonLinkFetchManySelectItems<S: Database> {
            pub select_items: Vec<DynamicCol>,
            pub dyn_from_row: Box<dyn DynFromRow<S::Row> + Send>,
        }

        impl<S: Database> StrAliased for JsonLinkFetchManySelectItems<S> {
            type StrAliased = Vec<AliasedCol<String, String, SanitizeMany<(&'static str, String)>>>;
            fn str_aliased(&self, alias: &'static str) -> Self::StrAliased {
                self.select_items
                    .iter()
                    .map(|e| AliasedCol {
                        table: e.table.clone(),
                        col: e.col.clone(),
                        alias: SanitizeMany((alias, e.col.clone())),
                    })
                    .collect()
            }
        }

        impl<S: Database> Clone for JsonLinkFetchManySelectItems<S> {
            fn clone(&self) -> Self {
                Self {
                    select_items: self.select_items.clone(),
                    dyn_from_row: self.dyn_from_row.clone_as_box(),
                }
            }
        }
        ...
        where
            S: DatabaseExt,
            T: Send,
            T: LinkFetchManyTakeId<SingleIncremintalInt<String>>,
            T: LinkFetchMany,
            T::SelectItems: 'static,
            T::SelectItems: Clone,
            T::SelectItems: MembersInDynamicOperation<S>,
            T::SelectItems:
            ...
            T::Output: Serialize,
        {
            ...
            where
                S: Database,
            {
                JsonLinkFetchManySelectItems {
                    select_items: self.non_aggregating_select_items().into_dynamic_cols(),
                    dyn_from_row: Box::new(self.non_aggregating_select_items()),
                }
            }
            fn join(&self) -> Box<dyn BoxedExpression<S> + Send> {
        ...
        impl<'a, S: Database> LinkFetchMany for Box<dyn JsonLinkFetchMany<S> + Send + 'a> {
            type Output = serde_json::Value;

            type SelectItems = JsonLinkFetchManySelectItems<S>;
...
pub mod fetch_one {
    ...
    use crate::json_client::supported_links_on_fetch_one::InvalidLink;
    ...
    pub enum FetchOneError {
        ...
        InvalidLink(u64, InvalidLink),
    }

    impl From<(u64, InvalidLink)> for FetchOneError {
        fn from(value: (u64, InvalidLink)) -> Self {
            Self::InvalidLink(value.0, value.1)
        }
    }
        ...
        for<'q> i64: Decode<'q, S> + Type<S>,
    {
        pub fn fetch_one(
            ...
            Output = Result<Option<LinkedOutput<i64, JsonValue, Vec<JsonValue>>>, FetchOneError>,
        > + Send {
            async move {
                let gaurd: RwLockReadGuard<'_, _> = self
                    .collections
